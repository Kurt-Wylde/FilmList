...
class Watched: UITableViewController {
    ...
    self.WatchedItems = try [WatchedItem].readFromPersistence()
    }
    ...
    try WatchedItems.writeToPersistence()
    }
    catch let error
    {
    ...
    func didTapAddItemButton(_ sender: UIBarButtonItem)
    {
        ...
        alert.addAction(UIAlertAction(title: "Confirm", style: .default, handler:
            { (_) in
                ...
                {
                    self.addNewWatchedItem(title: title)
                }
        }))
    ...
    private func addNewWatchedItem(title: String)
    {
    // The index of the new item will be the current item count
    let newIndex = WatchedItems.count
    
    // Create new item and add it to the films items list
    WatchedItems.append(WatchedItem(title: title))
    ...
    if indexPath.row < WatchedItems.count
    {
    let item = WatchedItems[indexPath.row]
    ...
    if indexPath.row < WatchedItems.count
    {
    WatchedItems.remove(at: indexPath.row)
